...
def start_tool_1():
    try:
        ...
        t1.app.run(host="0.0.0.0", port=5000, debug=False,
...
def start_tool_3():
    try:
        ...
        t3.load_schema()
        t3.app.run(host="0.0.0.0", port=7000, debug=False,
                   use_reloader=False, threaded=True)
    except Exception as e:
...
<style>
:root{
  --bg:#05080f;--bg2:#080d18;--surf:#0c1220;--surf2:#101929;
  --border:#182035;--border2:#1e2d45;--text:#8ba3c4;--bright:#dce8f8;
  --muted:#2d3f58;--muted2:#3d5570;
...
.nav-logo{font-family:'Syne',sans-serif;font-weight:800;font-size:15px;color:#fff;
          letter-spacing:3px;margin-right:32px;display:flex;align-items:center;
...
.nav-btn{position:relative;display:flex;align-items:center;gap:6px;padding:0 16px;
         font-size:10px;font-weight:700;letter-spacing:.5px;color:var(--muted2);
         cursor:pointer;border:none;background:none;font-family:'Space Mono',monospace;
         transition:color .2s;white-space:nowrap;border-bottom:2px solid transparent;margin-bottom:-1px}
...
.btn-fetch  .nav-dot{background:var(--fetch)}
.btn-ai     .nav-dot{background:var(--ai)}
...
.hero{padding:60px 32px 44px;text-align:center;position:relative}
.hero-glow{position:absolute;top:-40px;left:50%;transform:translateX(-50%);
           width:600px;height:300px;background:radial-gradient(ellipse,#22d87a08 0%,transparent 70%);pointer-events:none}
...
.hero h1{font-family:'Syne',sans-serif;font-size:clamp(26px,5vw,54px);font-weight:800;
...
.tool-card:hover::before{height:5px}
...
.card-port{font-size:9px;color:var(--muted2)}
.card-port b{font-weight:700}
.card-binlog .card-port b{color:var(--binlog)}
.card-fetch  .card-port b{color:var(--fetch)}
.card-ai     .card-port b{color:var(--ai)}
.card-open{display:flex;align-items:center;gap:5px;font-size:10px;font-weight:700;
           padding:6px 14px;border-radius:7px;border:1px solid;cursor:pointer;
           font-family:'Space Mono',monospace;transition:all .2s;background:none}
...
.offline-overlay{display:none;position:absolute;top:0;left:0;right:0;bottom:0;
                 ...
                 font-size:13px;color:#f43f5e;text-align:center}
  ...
  <div class="view active" id="view-home">
    <div class="home-scroll">
      ...
      <div class="tool-status-bar">
        ...
        <div class="ts-pill ts-checking" id="ts-fetch" onclick="showView('fetch')">
          <span class="ts-dot"></span>📂 File Reader — checking...
        </div>
        <div class="ts-pill ts-checking" id="ts-ai" onclick="showView('ai')">
      ...
      <div class="stats-bar">
        <div class="stat-pill">
          ...
          <div class="stat-info"><div class="stat-label">Databases</div><div class="stat-sub">adoptconvergebss + 9</div></div>
        </div>
        <div class="stat-pill">
          <div class="stat-val" style="color:var(--fetch)">3</div>
          <div class="stat-info"><div class="stat-label">Tools</div><div class="stat-sub">Monitor · Reader · AI</div></div>
        </div>
        <div class="stat-pill">
          <div class="stat-val" style="color:var(--ai)">CDC</div>
          <div class="stat-info"><div class="stat-label">Change Capture</div><div class="stat-sub">Zero-latency stream</div></div>
        </div>
        <div class="stat-pill">
          <div class="stat-val" style="color:#38bdf8">v2</div>
      ...
      <div class="tools-grid">
        <div class="tool-card card-binlog" onclick="showView('binlog')">
          ...
          <div class="card-footer">
            <div class="card-port">Port: <b>5000</b></div>
            <button class="card-open" onclick="event.stopPropagation();showView('binlog')">Open →</button>
          </div>
        </div>
        <div class="tool-card card-fetch" onclick="showView('fetch')">
          ...
          <div class="card-footer">
            <div class="card-port">Port: <b>7777</b></div>
            <button class="card-open" onclick="event.stopPropagation();showView('fetch')">Open →</button>
          </div>
        </div>
        <div class="tool-card card-ai" onclick="showView('ai')">
          ...
          <div class="card-footer">
            <div class="card-port">Port: <b>7000</b></div>
            <button class="card-open" onclick="event.stopPropagation();showView('ai')">Open →</button>
          </div>
        </div>
      </div>
      <footer>
...
def proxy_request(tool_name, path):
    ...
    try:
        ...
        return Response(
            f"""<html><body style="background:#05080f;color:#f43f5e;font-family:monospace;
            display:flex;align-items:center;justify-content:center;height:100vh;
            flex-direction:column;gap:12px;font-size:13px;text-align:center;padding:20px">
            <div style="font-size:40px">⚠️</div>
            <b>{tool_name} tool is starting up...</b>
...
if __name__ == "__main__":
    ...
    t1 = threading.Thread(target=start_tool_1, daemon=True)
    t1.start()
    time.sleep(1)

    t2 = threading.Thread(target=start_tool_2, daemon=True)
    t2.start()
    time.sleep(1)
